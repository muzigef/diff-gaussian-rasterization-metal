...
- 此工程是固定版本 CUDA → Metal 的迁移仓库；tile/scan/radix 与 MPS 调度迁移已实现。当前契约见 `docs/MIGRATION.md`，现有证据与未通过项见 `docs/VALIDATION.md` 和 `docs/TILE_MIGRATION_REPORT.md`；历史分析不作为当前实现描述。
...
- `src/metal/primitives.h` 与共享 shader 同时影响原生和 Torch 两条入口，改动后都要重新构建、验证。完整 Python 源码对照需相邻固定 CUDA 源码和 GLM；依赖及 skip 检查见 `docs/DEVELOPMENT.md`。
